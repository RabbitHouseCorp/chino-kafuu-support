...
const ignore = (process.argv.find((value) => value.startsWith('--ignore') || value.startsWith('--ignore=')) ?? '')
               .replace(/^--ignore=/g, '')
               .split(/,|\s+/)
...
export class Launcher extends EventEmitter {
  ...
  private async startResource() {
    const resources = this.resources
      ...
      .filter((value) => !(ignore.includes(value.getName) || ignore.find((str) => str.startsWith(value.getName.replace(/([-.])platform|([-.])service/g, '')))))
    if (ignore.length > 0) {
      const ignoreItems = this.resources
      .sort((a, b) => b.getType - a.getType)
      .filter((value) => (ignore.includes(value.getName) || ignore.find((str) => str.startsWith(value.getName.replace(/([-.])platform|([-.])service/g, '')))))
      .map((value) => value.getName)
      logger.debug(`Ignoring Platforms: ${JSON.stringify(ignoreItems, undefined, '')}`)

      ignoreItems.splice(0, ignoreItems.length)
    }
  ...
  public static default() {
    if (ignore.length > 0) {
      logger.warn('flagged an ignore flag for specific platforms.')
    }
    return new Launcher()
  }
}
