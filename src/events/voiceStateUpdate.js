			...
			if (server.animu) {
				if (newVoiceChannel) {
					if (newVoiceChannel.members.size >= 1) {
						if (newVoiceChannel.members.cache.get(this.client.user.id)) return
				...
				if (newVoiceChannel.members.cache.get(this.client.user.id) && newVoiceChannel.members.size === 1) {
